...
public class ProdServiceImpl implements IProdService {
	...
	public ServiceResult modifyProd(ProdVO prod) {
		...
		if (prodDAO.selectProd(prod.getProd_id())==null) {
			throw new CommonException(prod.getProd_id() + "에 해당하는 상품이 없습니다.");
		}
		if (prodDAO.updateProd(prod)>0) {
			result = ServiceResult.SUCCESS;
		}
