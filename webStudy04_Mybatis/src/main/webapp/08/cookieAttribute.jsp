...
<pre>
	...
	5. name/value(required) : RFC2396 규약에 따라 URLEncoding방식이 적용됨
	<%
		...
		allPathCookie.setPath(request.getContextPath()); // 절대경로 : 브라우저가 사용하기 때문에 client-side
		...
		otherPathCookie.setPath(request.getContextPath()+"/09"); // 절대경로 : 브라우저가 사용하기 때문에 client-side
		response.addCookie(otherPathCookie);
		
// 		Cookie otherDomainCookie = new Cookie("otherDomainCookie", "Other Domain Cookie~");
// 		otherDomainCookie.setDomain(".llh.com");
